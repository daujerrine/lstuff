...
// Returns the union of the two sets into the first one
int set_union(Set *a, Set *b)
{
    ...
    for (int i = 0; i < b->data_size; i++) {
        ...
        // This function is used only in calculation of the first and
        // follow sets only. So we put it here instead of in a separate
        // function.
        if (b->data[i] == '@') {
            continue;
        }
        ...
        if (!present) {
            int ret;
            if ((ret = set_insert(a, b->data[i])) < 0) {
                return ret;
            }
        }
    }
...
typedef struct ProductionSet {
    ...
    int nullable;
    int first_calculated;
    int follow_calculated;
} ProductionSet;
...
void production_set_init(ProductionSet *p, char symbol)
{
    ...
    p->nullable = 0;
    p->symbol = symbol;
    p->first_calculated = 0;
    p->follow_calculated = 0;
    set_init(&p->first_set);
    set_init(&p->follow_set);
}
...
void grammar_print(Grammar const * const g)
{
    ...
    for (int prod_set = 0; prod_set < g->size; prod_set++) {
        ...
        printf("  First Set: ");
        PRINT_COMMA_LIST(g->set[prod_set].first_set.data,
                         g->set[prod_set].first_set.data_size,
                         "%c");
        printf("\n");

        printf("  Follow Set: ");
        PRINT_COMMA_LIST(g->set[prod_set].follow_set.data,
                         g->set[prod_set].follow_set.data_size,
                         "%c");
        printf("\n\n");
    }
...
int grammar_set_starting_symbol(Grammar *g, char symbol)
{
...
int grammar_get_first_internal(Grammar *g, char nonterminal)
{
    int prodset = grammar_find(g, nonterminal);
    g->set[prodset].first_calculated = 1;
    int nullable;
    int epsilon = 0;
    int first_symbol = 0;
    char first_ch = 0;
    ...
    for (int prod = 0; prod < g->set[prodset].size; prod++) {
        first_ch = g->set[prodset].productions[prod].symbols[first_symbol];
        nullable = 1;

        // Determine epsilon
        if (g->set[prodset].productions[prod].size == 0) {
            set_insert(&g->set[prodset].first_set, '@');
            epsilon = 1;

        // Determine for nonterminal
        } else if (is_nonterminal(first_ch)) {

            // Iterate over entrire production until we reach the end or hit
            // a terminal.
            while (first_symbol < g->set[prodset].productions[prod].size) {
                int next_prodset = grammar_find(g, first_ch);

                if (!g->set[next_prodset].first_calculated) {
                    grammar_get_first_internal(g, first_ch);
                }

                set_union(&g->set[prodset].first_set, &g->set[next_prodset].first_set);

                if (!g->set[next_prodset].nullable) {
                    nullable = 0;
                    break;
                }
            ...
            set_insert(&g->set[prodset].first_set, first_ch);
        }
    }

    g->set[prodset].nullable = epsilon;

    return 0;
}

int grammar_get_first(Grammar *g)
{
    return grammar_get_first_internal(g, g->starting_symbol);
}

int grammar_get_follow_internal(Grammar *g)
{
    int nonterminal_found;
    int at_last;
    int continue_probing;

    // Repeat 2 times to grab circular production follows
    ...
    for (int prod_set = 0; prod_set < g->size; prod_set++) {
        for (int i_prod_set = 0; i_prod_set < g->size; i_prod_set++) {
            for (int prod = 0; prod < g->set[i_prod_set].size; prod++) {
                ...
                for (int symbol = 0;
                     symbol < g->set[i_prod_set].productions[prod].size;
                     symbol++) {

                    char curr_symbol = g->set[i_prod_set].productions[prod].symbols[symbol];

                    // Found the nonterminal?
                    if (!nonterminal_found &&
                        is_nonterminal(curr_symbol) &&
                        curr_symbol == g->set[prod_set].symbol) {
                    ...
                    // If not, go to next symbol
                    if (!nonterminal_found)
                        continue;

                    continue_probing = 0;

                    // Symbol next to nonterminal is a terminal?
                    if (is_terminal(curr_symbol)) {
                        // Achieved a terminal as follow. Add it and break.
                        at_last = 0;
                        set_insert(&g->set[prod_set].follow_set, curr_symbol);
                        break;
                    }

                    // Symbol next to terminal is a nonterminal
                    int target = grammar_find(g, curr_symbol);

                    set_union(&g->set[prod_set].follow_set, &g->set[target].first_set);

                    // If nullable, we need to continue probing to next symbol
                ...
                if (nonterminal_found && at_last) {
                    // This means the nonterminal is the last symbol of the
                    // prodduction.
                    // Copy over all follows of this production if it is not
                    // the current symbol. (i.e. prevent infinite recursion)
                    // If there are circular productions, their follows are
                    // grabbed in the next iteration of this whole nested loop.
                    if (g->set[i_prod_set].symbol != g->set[prod_set].symbol) {
                        set_union(&g->set[prod_set].follow_set, &g->set[i_prod_set].follow_set);
                    }
                }
            }
        }
...
int grammar_get_follow(Grammar *g)
{
    int prod_set = grammar_find(g, g->starting_symbol);
    set_insert(&g->set[prod_set].follow_set, '$');
    return grammar_get_follow_internal(g);
}
...
int grammar_create_lr_table_internal(Grammar *g, ItemSet *set, LR0ParsingTable *table, int item_offset)
{
    ...
    if (no_outgoing == 1) {
        ...
        if (item->productions[0].nonterminal == 'Z') {
            ...
            int prod_set = grammar_find(g, item->productions[0].nonterminal);
            // TODO handle 'grammar LR0 or not' case here
            for (int i = 0; i < g->set[prod_set].follow_set.data_size; i++) {
                int column = set_order(&g->terminal_set, g->set[prod_set].follow_set.data[i]);
                if (table->action_table[item_offset][column].type != ACTION_ERROR) {
                    printf("Reduce Overlap on I%d,'%c' %c%d -> R%d\n",
                    item_offset,
                    g->set[prod_set].follow_set.data[i],
                    ((table->action_table[item_offset][column].type == ACTION_SHIFT) ? 'S' : 'R'),
                    table->action_table[item_offset][column].target_state,

                    grammar_get_order_from_item(g, &item->productions[0]));
                }
                table->action_table[item_offset][column].type = ACTION_REDUCE;
                table->action_table[item_offset][column].target_state = grammar_get_order_from_item(g, &item->productions[0]);
                table->action_table[item_offset][column].nonterminal = item->productions[0].nonterminal;
                table->action_table[item_offset][column].production = item->productions[0].production;
            }
    ...
    for (int i = 0; i < item->productions_size; i++) {
        ...
        if (item->productions[i].dot_offset >= p->size) {
            if (item->productions[i].nonterminal == 'Z') {
                ...
                int prod_set = grammar_find(g, item->productions[i].nonterminal);
                // TODO handle 'grammar LR0 or not' case here
                for (int j = 0; j < g->set[prod_set].follow_set.data_size; j++) {
                    int column = set_order(&g->terminal_set, g->set[prod_set].follow_set.data[j]);
                    if (table->action_table[item_offset][j].type != ACTION_ERROR) {
                        printf("Reduce Overlap on I%d,'%c' %c%d -> R%d\n",
                        item_offset,
                        g->set[prod_set].follow_set.data[j],
                        ((table->action_table[item_offset][column].type == ACTION_SHIFT) ? 'S' : 'R'),
                        table->action_table[item_offset][column].target_state,
                    ...
                    table->action_table[item_offset][column].type = ACTION_REDUCE;
                    table->action_table[item_offset][column].target_state = grammar_get_order_from_item(g, &item->productions[i]);
                    table->action_table[item_offset][column].nonterminal = item->productions[i].nonterminal;
                    table->action_table[item_offset][column].production = item->productions[i].production;
                }
            }
        ...
        // printf("Dealing With: ");
        // item_production_print(g, &item->productions[i]);
        // printf(" dot offset = %d, size = %d", item->productions[i].dot_offset , p->size);
        // printf("\n");
        ...
        if (next < 0) {
            ...
            for (int j = i + 1; j < item->productions_size; j++) {
                ...
                if (extra_p->symbols[item->productions[j].dot_offset] ==  p->symbols[item->productions[i].dot_offset]) {
                    //printf("Non closure size greater than %d for I%d.", new_item->productions_size, next);
                    //item_production_print(g, &item->productions[j]);
                    //printf("\n");
        ...
        if (is_nonterminal(p->symbols[item->productions[i].dot_offset])) {
            ...
            if (table->action_table
               ...
               [set_order(&g->terminal_set, p->symbols[item->productions[i].dot_offset])].type != ACTION_ERROR) {
                printf("Shift Overlap on I%d,'%c' %c%d -> S%d\n",
                    item_offset,
                    p->symbols[item->productions[i].dot_offset],
                    ((table->action_table[item_offset][set_order(&g->terminal_set, p->symbols[item->productions[i].dot_offset])].type == ACTION_SHIFT) ? 'S' : 'R'),
                    table->action_table[item_offset][set_order(&g->terminal_set, p->symbols[item->productions[i].dot_offset])].target_state,
                    next);
            }
